<template>
    <div @keydown="handleKeydown" tabindex="0" class="page-container">
        <div ref="aaa">
            父元素
            <div ref="bbb">子元素</div>
            <a-select ref="ccc" placeholder="Select" style="width: 150px;margin-top: 16px;">
        ...
        >
            <!-- 单元格内的组件 -->
            <a-input v-model="cell.inputValue" ref="input" placeholder="Input Field" />
            <a-select v-model="cell.selectedValue" ref="select" placeholder="Select" style="width: 150px;margin-top: 16px;" @click="selectClick">
                <a-select-option value="jack">Jack</a-select-option>
            </a-select>
        </div>
    </div>
...
export default {
    ...
    mounted() {
        // const aRef = this.$refs['aaa'];
        // console.log('aRef', aRef);
        // const cRef = this.$refs['ccc'];
        // console.log('cRef', cRef.$el);
        // const bRef = this.$refs['aaa'].$refs['bbb'];
    ...
    methods: {
        ...
        focusComponent(cellIndex, componentIndex) {
            ...
            console.log('component', component, component.classList);
            const childCom = this.$refs[cellRef][0];
            ...
            if (component) {
                ...
                if (component.classList.contains('ant-select')) {
                    // 获取下拉框的触发按钮
                    const selectTrigger = component.querySelector('.ant-select-selection');
